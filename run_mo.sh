...
# Config options
workflow_data=$WORKFLOW_DATA_MONTAGE2_3_0_LARGEST
...
quotaRequestsCpu="96" # 6 # 15 # 25 # 50
quotaRequestsMem="300Gi" # 12 # 25Gi # 100 # 200
...
elif [[ $nodes_size_selected = "cyfronet" ]]; then
    machine_type="c3.xmedium"
    num_nodes="19"
    quotaRequestsCpu="96" # 6 # 15 # 25 # 50
    quotaRequestsMem="300Gi" # 12 # 25Gi # 100 # 200
...
if [[ $requests_selected = "100" ]]; then
    workerRequestsCpu="0.10"
    workerRequestsMem="262144000"
elif [[ $requests_selected = "250" ]]; then
    workerRequestsCpu="0.25"
    workerRequestsMem="262144000"
elif [[ $requests_selected = "500" ]]; then
workerRequestsCpu="0.5"
workerRequestsMem="262144000"
elif [[ $requests_selected = "700" ]]; then
    workerRequestsCpu="0.7"
    workerRequestsMem="500000000"
...
delete_job() {
    if kubectl -n workerpools get "$1" >/dev/null 2>&1 ; then
        kubectl -n workerpools delete "$1"
    fi
}

purge_data() {
    #kubectl -n workerpools delete job logs-parser
    #kubectl -n workerpools delete job viz-exec-trace

    engine_pod_name="$(kubectl get pods -n workerpools --no-headers -o custom-columns=":metadata.name" | grep hyperflow-engine)"
    kubectl -n workerpools exec -it $engine_pod_name -- sh -c 'find /work_dir -not -name "work_dir" -delete'
    #kubectl -n workerpools exec -it $engine_pod_name -- sh -c 'rm -rf /work_dir/*'
    ...
    delete_job job/nfs-data
    helm upgrade -i hyperflow-nfs-data --namespace workerpools charts/hyperflow-nfs-data --values values/cluster/hyperflow-nfs-data.yaml --set workflow.image=$workflow_data
...
run_workflow() {
    engine_pod_name="$(kubectl get pods -n workerpools --no-headers -o custom-columns=":metadata.name" | grep hyperflow-engine)"
    helm upgrade -i $workflow_name-workers --namespace workerpools workflow_charts/$workflow_name/resources --values workflow_charts/$workflow_name/resources/values.yaml --set workerRequests.cpu=\"$workerRequestsCpu\",workerRequests.memory=\"$workerRequestsMem\",quotaRequests.cpu=\"$quotaRequestsCpu\",quotaRequests.memory=\"$quotaRequestsMem\"
    helm upgrade -i $workflow_name-vpa --namespace workerpools workflow_charts/$workflow_name/vpa --values workflow_charts/$workflow_name/vpa/values.yaml --set updateMode=$vpa_option
...
fetch_metrics() {
    ...
    #kubectl -n workerpools delete job 
    delete_job job/logs-parser
